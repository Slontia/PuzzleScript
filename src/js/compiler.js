...
function autoGenerateCollectionDirections(state) {
    ...
    // Collect all directional definitions (both object and collection level)
    for (let entry of state.legend_directionals) {
        ...
        let down = entry[3];
        let left = entry[4];
        let variants = [base, right, down, left];
        
        // Map all variants to the same array
    ...
    for (let i = 0; i < state.legend_directionals.length; i++) {
        ...
        let isCollection = entry.length > 5 && entry[5] === true;
        
        if (!isCollection) {
            continue; // Only process collection directionals
        }
        ...
        let rightOriginal = state.original_case_names[right] || right;
        ...
        for (let prop of state.legend_properties) {
            if (prop[0] === base) {
                baseCollectionDef = prop;
                break;
            }
        }

        if (!baseCollectionDef) {
            logError(`Collection "${base.toUpperCase()}" is not defined in LEGEND.`, lineNumber);
            continue;
        }
        ...
        for (let prop of state.legend_properties) {
            if (prop[0] === right) rightExists = true;
            if (prop[0] === down) downExists = true;
            if (prop[0] === left) leftExists = true;
        }
        ...
        if (!rightExists) {
            ...
            if (rotatedObjects && rotatedObjects.length > 0) {
                ...
                // Also register original case name
                if (rightOriginal && !state.original_case_names[right]) {
        ...
        if (!downExists) {
            let rotatedObjects = rotateCollectionObjects(baseObjects, 2, variantsMap, state);
            if (rotatedObjects && rotatedObjects.length > 0) {
                let rotatedDef = [down].concat(rotatedObjects);
                rotatedDef.lineNumber = lineNumber;
                state.legend_properties.push(rotatedDef);
                ...
                // Also register original case name
                if (downOriginal && !state.original_case_names[down]) {
                    state.original_case_names[down] = downOriginal;
                }
            } else {
                logError(`Cannot auto-generate collection "${downOriginal.toUpperCase()}" - objects in "${base.toUpperCase()}" don't have directional definitions.`, lineNumber);
            }
        }

        if (!leftExists) {
            let rotatedObjects = rotateCollectionObjects(baseObjects, 3, variantsMap, state);
            if (rotatedObjects && rotatedObjects.length > 0) {
                let rotatedDef = [left].concat(rotatedObjects);
                rotatedDef.lineNumber = lineNumber;
                state.legend_properties.push(rotatedDef);
                ...
                // Also register original case name
                if (leftOriginal && !state.original_case_names[left]) {
                    state.original_case_names[left] = leftOriginal;
                }
            } else {
                logError(`Cannot auto-generate collection "${leftOriginal.toUpperCase()}" - objects in "${base.toUpperCase()}" don't have directional definitions.`, lineNumber);
            }
        }
        
        // After generating all directions, add the alias definition (alias = base or right or down or left)
        // Check if alias definition already exists
...
function rotateCollectionObjects(baseObjects, rotations, variantsMap, state) {
    // Rotate collection objects using the variantsMap
    ...
    function tryRotateByName(objName, rotations) {
        // Try to rotate object name by rotating direction suffixes
        // e.g., MirrorSlashLaserRU (Right-Up) -> MirrorSlashLaserDR (Down-Right)
        const lower = objName.toLowerCase();
        
        // Check if name ends with two direction letters
        const match = lower.match(/^(.+?)([udlr])([udlr])$/);
        if (match) {
            const [, prefix, dir1, dir2] = match;
            const newDir1 = rotateDir(dir1, rotations);
            const newDir2 = rotateDir(dir2, rotations);
            const rotatedName = prefix + newDir1 + newDir2;
            
            // Check if this object exists in the game
        ...
        // Check if name ends with one direction letter
        const match1 = lower.match(/^(.+?)([udlr])$/);
        if (match1) {
            const [, prefix, dir] = match1;
            const newDir = rotateDir(dir, rotations);
            const rotatedName = prefix + newDir;
    ...
    for (let obj of baseObjects) {
        // First try name-based rotation
        let rotatedObject = tryRotateByName(obj, rotations);
        ...
        if (!rotatedObject) {
            ...
            // Find which variant this object is (0=up, 1=right, 2=down, 3=left)
            let currentIndex = -1;
...
function preprocessDirectionalLegends(state) {
    ...
    for (let i = 0; i < state.legend_directionals.length; i++) {
        ...
        if (isCollection) {
            continue; // Skip collection directionals - they're handled by autoGenerateCollectionDirections
        }
        ...
        // Do NOT automatically add rotated objects to the base object's collision layer
        // This allows users to place them in different layers explicitly
        // The old behavior forced all rotated variants into the same layer as the base
        // which prevented separation of directional laser beams, etc.
...
// Assign auto-created directional objects to their referenced layers
// This ensures objects created by @ directives are assigned to appropriate collision layers
// based on which property references them in COLLISIONLAYERS
function assignAutoCreatedObjectsToLayers(state) {
    // Build a map of which layer each property is in
    let propertyLayerMap = {};
    for (let layerIdx = 0; layerIdx < state.collisionLayers.length; layerIdx++) {
        for (let i = 0; i < state.collisionLayers[layerIdx].length; i++) {
            let name = state.collisionLayers[layerIdx][i];
            propertyLayerMap[name] = layerIdx;
        }
    }
    
    // For each property, check its constituent objects
    for (let propIdx = 0; propIdx < state.legend_properties.length; propIdx++) {
        let prop = state.legend_properties[propIdx];
        let propName = prop[0];
        
        // Check if this property itself is in a collision layer
        if (propertyLayerMap[propName] === undefined) {
            continue; // Property not in any layer, skip it
        }
        
        let targetLayer = propertyLayerMap[propName];
        
        // For each object referenced by this property
        for (let objIdx = 1; objIdx < prop.length; objIdx++) {
            let objName = prop[objIdx];
            
            // Check if this object exists but is not in any collision layer
            if (state.objects[objName] !== undefined) {
                // Check if object is already in some layer
                let isInLayer = false;
                for (let layerIdx = 0; layerIdx < state.collisionLayers.length; layerIdx++) {
                    if (state.collisionLayers[layerIdx].indexOf(objName) !== -1) {
                        isInLayer = true;
                        break;
                    }
                }
                
                // If not in any layer, add it to the property's layer
                if (!isInLayer) {
                    if (state.collisionLayers[targetLayer].indexOf(objName) === -1) {
                        state.collisionLayers[targetLayer].push(objName);
                    }
                }
            }
        }
...
function expandDirectionalRules(rule, state) {
    ...
    */
    ...
    // Extract and remove modifiers (late, rigid, random, +) from the beginning
    // Note: We only extract actual rule modifiers, not directional keywords
    let modifiers = '';
    let ruleWithoutModifiers = ruleString;
    const modifierPattern = /^(\+?\s*(?:late|rigid|random)\s+)*/i;
    const matchModifiers = ruleString.match(modifierPattern);
    if (matchModifiers && matchModifiers[0]) {
        modifiers = matchModifiers[0].trim();
        ruleWithoutModifiers = ruleString.substring(matchModifiers[0].length).trim();
    }
    
    // Check if rule starts with @direction 
    let baseRule = ruleWithoutModifiers;
    ...
    for (let dir = 0; dir < 4; dir++) {
        ...
        if (baseRule.toLowerCase().startsWith(pattern + ' ')) {
            ...
            // Keep the @direction marker in baseRule - we need it for rotation!
            // Don't remove it here; it will be replaced during the rotation loop
            break;
        }
    }
    ...
    const collectionVariantsMap = {}; // Maps collection names to their directional variants
    
    if (state.legend_directionals && state.legend_directionals.length > 0) {
        for (let entry of state.legend_directionals) {
            const isCollection = entry.length > 5 && entry[5] === true;
            const [alias, base, right, down, left] = entry;
            const variants = [base, right, down, left];
            
            if (isCollection) {
                // For collection directional definitions
                // Map the alias to the collection variants
                collectionVariantsMap[alias] = variants;
                collectionVariantsMap[base] = variants;
                collectionVariantsMap[right] = variants;
                collectionVariantsMap[down] = variants;
                collectionVariantsMap[left] = variants;
            } else {
    ...
    for (let rotation = 0; rotation < 4; rotation++) {
        ...
        // Build a map of direction replacements for this rotation
        // Key: @direction, Value: target direction (without @)
        const directionReplacements = {};
        for (let dir = 0; dir < 4; dir++) {
            const fromDir = directions[dir];
            const toDir = directions[(dir + rotation) % 4];
            directionReplacements['@' + fromDir] = toDir;
        }
        
        // Track which direction will be at the start if rule has @direction marker
        let startDirection = '';
        if (hasDirectionMarker && startDirectionIndex >= 0) {
            // Get the rotated direction that will be at the start
            startDirection = directionReplacements['@' + directions[startDirectionIndex]];
        }
        
        // Replace @direction markers with a placeholder first to avoid interference
        // Use unique markers that won't appear elsewhere
        // But SKIP @direction at the beginning if it's a start marker - we'll handle it separately
        const placeholders = {};
        for (let key in directionReplacements) {
            // Skip replacing the @direction at the start of the rule - we'll handle it with startDirection
            if (hasDirectionMarker && key === '@' + directions[startDirectionIndex]) {
                // Don't replace this one, we'll remove it entirely and add it back later
                const placeholder = '___DIRECTION_STARTMARKER_' + startDirectionIndex + '___';
                placeholders[key] = placeholder;
                rotatedRule = rotatedRule.replace(new RegExp('^' + key + '\\s+'), placeholder);
            } else {
                const placeholder = '___DIRECTION_' + directionReplacements[key].toUpperCase() + '___';
                placeholders[key] = placeholder;
                rotatedRule = rotatedRule.replace(new RegExp(key + '\\b', 'gi'), placeholder);
            }
        }
        
        // Now replace placeholders with the actual directions (without @)
        for (let key in placeholders) {
            const placeholder = placeholders[key];
            if (placeholder.includes('STARTMARKER')) {
                // Remove the start marker entirely - it will be added back via startDirection
                rotatedRule = rotatedRule.split(placeholder).join('');
            } else {
                rotatedRule = rotatedRule.split(placeholder).join(directionReplacements[key]);
            }
        }
        
        // Replace all @ObjectName and @CollectionName with rotated variants
        // Use group-level replacements to avoid chain-rotating variants in the same group.
        const variantGroups = [];
        ...
        const addVariantGroup = function (variants) {
            if (!variants || variants.length !== 4) return;
            const key = variants.join('|');
            if (seenGroupKeys.has(key)) return;
            seenGroupKeys.add(key);
            variantGroups.push(variants);
        };

        Object.values(objectVariantsMap).forEach(addVariantGroup);
        Object.values(collectionVariantsMap).forEach(addVariantGroup);

        for (let variants of variantGroups) {
            const nameToIndex = {};
            for (let i = 0; i < variants.length; i++) {
                nameToIndex[variants[i].toLowerCase()] = i;
            }

            const sortedVariants = variants.slice().sort((a, b) => b.length - a.length);
            const pattern = new RegExp('@(' + sortedVariants.map(escapeRegExp).join('|') + ')\\b', 'gi');
            rotatedRule = rotatedRule.replace(pattern, function (match, name) {
                const currentIdx = nameToIndex[name.toLowerCase()];
        ...
        // Add the rule direction if present
        if (startDirection.length > 0) {
            rotatedRule = startDirection + ' ' + rotatedRule;
        }
        
        // Add modifiers at the very beginning
        if (modifiers.length > 0) {
            rotatedRule = modifiers + ' ' + rotatedRule;
        }
...
function loadFile(str) {
    ...
    // Assign auto-created directional objects to their referenced layers
    assignAutoCreatedObjectsToLayers(state);
